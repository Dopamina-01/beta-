const STORAGE_KEY = 'dopamina-app-state-v1';
const DEFAULT_SETTINGS = {
  companyName: 'DOPAMINA',
  dailyTarget: 1000,
  monthlyTarget: 30000,
  logoUrl: '',
  primaryColor: '#3b82f6',
...
function bindEvents() {
  ...
  [els.dashboardDate, els.dashboardDay, els.dashboardNight, els.dashboardObservations].forEach((input) => {
    input.addEventListener('input', updateDashboardTotals);
  });
  ...
  els.themeToggle.addEventListener('change', () => {
    state.darkMode = els.themeToggle.checked;
    applyTheme();
    saveState();
  });
...
function renderHistory() {
  const filters = {
    search: els.historySearch.value.trim().toLowerCase(),
    month: els.historyMonth.value,
    year: els.historyYear.value,
    sort: els.historySort.value,
  };
  ...
  if (!list.length) {
    els.historyTableBody.innerHTML = '<tr><td colspan="6">No hay registros para mostrar.</td></tr>';
    return;
  }
  ...
  els.historyTableBody.innerHTML = list.map((record) => `
    <tr>
      ...
      <td>
        <button class="action-btn edit-btn" data-edit="${record.id}">Editar</button>
        <button class="action-btn delete-btn" data-delete="${record.id}">Eliminar</button>
      </td>
    </tr>
  `).join('');
...
function renderCharts() {
  ...
  els.chartsGrid.innerHTML = [
    renderBarChart('Producción por día', dailyData, 'date', 'value'),
    renderBarChart('Producción por semana', weeklyData, 'label', 'value'),
    renderBarChart('Producción por mes', monthlyData, 'label', 'value'),
    renderBarChart('Producción por año', yearlyData, 'label', 'value'),
    renderBarChart('Comparación entre turnos', shiftComparison, 'label', 'value'),
    renderBarChart('Comparación entre meses', monthComparison, 'label', 'value'),
  ].join('');
}

function renderBarChart(title, data, labelKey, valueKey) {
  if (!data.length) {
    return `<div class="chart-card"><h4>${title}</h4><p class="muted">Sin datos aún.</p></div>`;
  }
  const maxValue = Math.max(...data.map((item) => item[valueKey]), 1);
  const bars = data.map((item) => {
    const height = Math.max((item[valueKey] / maxValue) * 100, 6);
    return `
...
function renderSettings() {
  ...
  if (state.settings.logoUrl) {
    els.logoPreview.innerHTML = `<img src="${state.settings.logoUrl}" alt="Logo" />`;
  } else {
    els.logoPreview.innerHTML = '<span class="muted">Sube un logo para la empresa.</span>';
  }
}
...
function populateDateControls() {
  ...
  fillSelect(els.historyMonth, months);
  fillSelect(els.historyYear, years);
  fillSelect(els.exportMonth, months);
  fillSelect(els.exportYear, years);
  fillSelect(els.exportYearOnly, years);
  const today = new Date().toISOString().slice(0, 10);
  els.exportDayDate.value = today;
...
function saveRecord() {
  ...
  const dayProduction = parseFloat(els.dashboardDay.value) || 0;
  const nightProduction = parseFloat(els.dashboardNight.value) || 0;
  const totalDaily = dayProduction + nightProduction;
  const observations = els.dashboardObservations.value.trim();
  const now = new Date().toISOString();

  if (!date) {
    setStatus('Selecciona una fecha válida.');
    return;
  }
  ...
  const record = {
    id: els.editingRecordId.value || createId(),
    date,
    dayProduction,
    nightProduction,
    totalDaily,
    ...
    observations,
    createdAt: existingIndex >= 0 ? state.records[existingIndex].createdAt : now,
    updatedAt: now,
  };
  ...
  if (existingIndex >= 0) {
    ...
    state.records[duplicateIndex] = record;
  } else {
...
function editRecord(id) {
  ...
  els.dashboardDate.value = record.date;
  els.dashboardDay.value = record.dayProduction;
  els.dashboardNight.value = record.nightProduction;
  els.dashboardObservations.value = record.observations || '';
  updateDashboardTotals();
  els.saveRecordBtn.textContent = 'Actualizar registro';
...
function resetForm() {
  ...
  els.dashboardDate.value = new Date().toISOString().slice(0, 10);
  els.dashboardDay.value = '';
  els.dashboardNight.value = '';
  els.dashboardObservations.value = '';
  els.saveRecordBtn.textContent = 'Guardar registro';
  updateDashboardTotals();
}

function updateDashboardTotals() {
  const day = parseFloat(els.dashboardDay.value) || 0;
  const night = parseFloat(els.dashboardNight.value) || 0;
  const total = day + night;
  els.dashboardTotal.textContent = formatNumber(total);
  const monthKey = els.dashboardDate.value.slice(0, 7);
  els.dashboardMonthTotal.textContent = formatNumber(getMonthTotal(monthKey));
}

function saveSettings() {
  state.settings.companyName = els.companyName.value.trim() || DEFAULT_SETTINGS.companyName;
  state.settings.dailyTarget = parseFloat(els.dailyTarget.value) || 0;
  state.settings.monthlyTarget = parseFloat(els.monthlyTarget.value) || 0;
  state.settings.primaryColor = els.primaryColor.value;
  state.settings.secondaryColor = els.secondaryColor.value;
  saveState();
  applyTheme();
...
function handleLogoUpload(event) {
  ...
  reader.onload = () => {
    state.settings.logoUrl = reader.result;
    saveState();
    renderSettings();
...
function backupDatabase() {
  ...
  link.href = url;
  link.download = 'dopamina-backup.json';
  link.click();
...
function restoreDatabase(event) {
  ...
  reader.onload = () => {
    try {
      ...
      setStatus('No se pudo restaurar el respaldo.');
    }
  };
...
function exportData(type, format) {
  let records = [];
  if (type === 'day') {
    const date = els.exportDayDate.value;
    records = state.records.filter((record) => record.date === date);
  } else if (type === 'month') {
    const month = els.exportMonth.value;
    records = state.records.filter((record) => record.date.slice(0, 7) === month);
  } else if (type === 'year') {
    const year = els.exportYearOnly.value;
    records = state.records.filter((record) => record.date.slice(0, 4) === year);
  } else if (type === 'range') {
    const start = els.exportStartDate.value;
    const end = els.exportEndDate.value;
    records = state.records.filter((record) => record.date >= start && record.date <= end);
  }

  if (!records.length) {
    setStatus('No hay datos para exportar en este periodo.');
    return;
  }
...
function downloadCsv(records) {
  const header = ['Fecha', 'Día', 'Noche', 'Total', 'Observaciones'];
  const rows = records.map((record) => [record.date, record.dayProduction, record.nightProduction, record.totalDaily, (record.observations || '').replace(/\n/g, ' ')]);
  const csv = [header, ...rows].map((row) => row.map((value) => `"${String(value).replace(/"/g, '""')}"`).join(',')).join('\n');
  downloadText(csv, 'dopamina.csv');
}

function downloadExcel(records) {
  const rows = records.map((record) => `\n<tr><td>${record.date}</td><td>${record.dayProduction}</td><td>${record.nightProduction}</td><td>${record.totalDaily}</td><td>${escapeHtml(record.observations || '')}</td></tr>`).join('');
  const xml = `<?xml version="1.0"?><?mso-application progid="Excel.Sheet"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Worksheet ss:Name="DOPAMINA"><Table><Row><Cell><Data ss:Type="String">Fecha</Data></Cell><Cell><Data ss:Type="String">Día</Data></Cell><Cell><Data ss:Type="String">Noche</Data></Cell><Cell><Data ss:Type="String">Total</Data></Cell><Cell><Data ss:Type="String">Observaciones</Data></Cell></Row>${rows}</Table></Worksheet></Workbook>`;
  downloadText(xml, 'dopamina.xls');
}

function printPdf(records) {
  const content = `
    <html>
      <body style="font-family: Arial; padding: 24px;">
        <h1>DOPAMINA</h1>
        <p>Reporte exportado</p>
        <table style="width:100%; border-collapse:collapse;">
          <thead>
            <tr><th style="text-align:left; border-bottom:1px solid #ccc;">Fecha</th><th style="text-align:left; border-bottom:1px solid #ccc;">Día</th><th style="text-align:left; border-bottom:1px solid #ccc;">Noche</th><th style="text-align:left; border-bottom:1px solid #ccc;">Total</th></tr>
          </thead>
          <tbody>
            ${records.map((record) => `<tr><td>${record.date}</td><td>${record.dayProduction}</td><td>${record.nightProduction}</td><td>${record.totalDaily}</td></tr>`).join('')}
          </tbody>
        </table>
      </body>
    </html>
...
function activateSection(section) {
  ...
  document.querySelectorAll('.section').forEach((panel) => panel.classList.toggle('active-section', panel.id === `${section}Section`));
  els.pageTitle.textContent = section === 'dashboard' ? 'Dashboard' : section === 'history' ? 'Historial' : section === 'stats' ? 'Estadísticas' : section === 'charts' ? 'Gráficos' : section === 'exports' ? 'Exportaciones' : 'Configuración';
}

function applyTheme() {
  const root = document.documentElement;
  root.style.setProperty('--accent', state.settings.primaryColor);
  root.style.setProperty('--accent-2', state.settings.secondaryColor);
  document.body.style.background = state.darkMode ? `linear-gradient(135deg, ${state.settings.primaryColor}11, ${state.settings.secondaryColor}11)` : '#ffffff';
  document.body.style.color = state.darkMode ? 'var(--text)' : '#111827';
}
...
function getTodayTotal() {
  ...
  return state.records.filter((record) => record.date === date).reduce((sum, record) => sum + record.totalDaily, 0);
}

function getMonthTotal(monthKey) {
  return state.records.filter((record) => record.date.slice(0, 7) === monthKey).reduce((sum, record) => sum + record.totalDaily, 0);
}

function getAverageDaily() {
  if (!state.records.length) return 0;
  return state.records.reduce((sum, record) => sum + record.totalDaily, 0) / state.records.length;
}

function getBestDay() {
  return state.records.reduce((best, record) => (record.totalDaily > (best.totalDaily || -Infinity) ? record : best), {});
}

function getWorstDay() {
  return state.records.reduce((worst, record) => (record.totalDaily < (worst.totalDaily || Infinity) ? record : worst), {});
}

function getMonthSummary() {
  const monthly = {};
  state.records.forEach((record) => {
    monthly[record.date.slice(0, 7)] = (monthly[record.date.slice(0, 7)] || 0) + record.totalDaily;
  });
  const months = Object.entries(monthly).map(([label, value]) => ({ label, value }));
  months.sort((a, b) => b.value - a.value);
  const currentMonth = new Date().toISOString().slice(0, 7);
  const currentMonthTotal = monthly[currentMonth] || 0;
  return {
    currentMonthTotal,
    bestMonth: months[0] || { label: '', value: 0 },
    worstMonth: months[months.length - 1] || { label: '', value: 0 },
  };
}

function buildDailySeries() {
  const lastDays = state.records.slice(-10).map((record) => ({ label: record.date.slice(5), value: record.totalDaily }));
  return lastDays;
}
...
function buildWeeklySeries() {
  ...
  state.records.forEach((record) => {
    ...
    const key = `S${Math.ceil((date.getDate() + 6 - date.getDay()) / 7)}`;
    weekly[key] = (weekly[key] || 0) + record.totalDaily;
  });
...
function buildYearlySeries() {
  const yearly = {};
  state.records.forEach((record) => {
    yearly[record.date.slice(0, 4)] = (yearly[record.date.slice(0, 4)] || 0) + record.totalDaily;
  });
  return Object.entries(yearly).map(([label, value]) => ({ label, value }));
}
